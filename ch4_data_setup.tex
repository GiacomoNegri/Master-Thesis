...
\section{Training Configuration OLD}
...
\textbf{Noise} \quad The values of \(\sigma_{\max}\) and \(\sigma_{\min}\) vary depending on the SDE
...
\(\sigma_{\min} = 0.1\). For the VP formulation, the noise schedule is instead parameterised by \(\beta_{\max} = 8\) and \(\beta_{\min} = 0.01\). Two further parameters relevant specifically to Phase 2 are \(P_{\text{mean}}\) and
...
% flow ODE, Eq.~\ref{eq:probability_flow}. Being deterministic, it produces 
