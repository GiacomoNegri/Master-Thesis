...
Specifically, the list of current S\&P 500 members was taken as the starting universe (as of 10th of April 2026), and
...
and corporate failures. The second aspect regards the pre-decimalization process, which was mandated by the Securities and Exchange Commission in January 2000 and completed by the 9th of April 2001 \cite{furfine2003decimalization}. Before this period, US stock prices were quoted in fractions, specifically 1/16th of a dollar, meaning that the minimum price movement needed to record an effective change in price was 0.0625 dollars. After decimalization, this changed to 1 cent. This manifests as 7.1928\% of all returns displaying an exact 0\% daily return (in depth analysis in sub-Sec.\ \ref{app:subsec:decimalization}).

\section{Preprocessing}

\textbf{Phase~1} \quad Only univariate log-returns are used, as defined in Eq.~\ref{eq:log_returns}, and no preprocessing is applied to them since no mention of it is made by Kim et al. \cite{kim2025diffusion}, and the average log-return \(\mu=0.0005\) does not conflict with prior sampling assumptions.

\textbf{Phase~2} \quad \All four OHLC channels are required, the Open, High,
...
\begin{itemize}
    ...
    \item \textbf{Phase 2, EDM-CSDI:} window length \(L = 512\) and stride \(s = 100\), yielding \(25,226\) maximum possible windows with no validation \(\texttt{val_split_ratio}=0\).
\end{itemize}
...
constraints imposed by MIG partitioning of the A100 GPU by the university's High-Performance Computing policy. The full memory analysis is given in Sec.~\ref{app:sec:attention_memory}.
...
% \textbf{Phase 2} \quad In order to assess the discriminative power of the conditioning information, validation windows were fully held out: for any ticker whose window was selected for validation, no other window covering the same time range appeared in the training set. This reduces the number of training samples available for learning, since for every additional validation window used 6 training windows must be suppressed; however, this effect is mitigated by the larger number of windows available.
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/ch4/noise_schedules.png}
    ...
    \begin{subfigure}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/ch4/edm_noise_distribution.png}
