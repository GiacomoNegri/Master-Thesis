...
\begin{figure}[htbp]          % placement: here, top, bottom, page
  ...
  \caption{Phase~2 selection of Validation windows process. Example with window length \(L=4\), stride \(s=1\), and \texttt{val\_split\_ratio}\(=0.25\), where the \(7^{\text{th}}\) index of the ticker (red block) is extracted randomly.}
  \label{fig:window_construction}
\end{figure}

\textbf{Phase 2} \quad In order to assess the discriminative power of the conditioning information, validation windows were fully held out: for each ticker a contiguous block of records amounting to a \texttt{val\_split\_ratio} fraction was reserved starting at a random index, and every window straddling this interval was excluded from the subsequent training. This per-ticker approach enables our model to see diverse market conditions across all stocks without concentrating only on the longer-lasting tickers. The validation split ratio was set to 5\% (239 windows), because, since windows overlap due to the stride, each additional validation window required roughly 10 training windows to be discarded. For 5\% validation split \(2,129\) windows are discarded, leaving \(22,858\) for training. For instance windows that fell entirely within the validation block, by contrast, were retained and used in full as validation.
...
\begin{figure}[htbp]
    ...
    \begin{subfigure}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/ch4/noise_schedules.jpg}
    ...
    \begin{subfigure}{0.48\textwidth}
        \centering
        \includegraphics[width=\textwidth]{images/ch4/edm_noise_distribution.jpg}
...
the VP formulation is parameterised by $\beta_{\max} = 8$ and $\beta_{\min} = 0.01$. The choice was reached after testing the forward process and reaching prior matching while retaining as much signal as possible, with focus on the first.
