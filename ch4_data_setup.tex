...
\textbf{Phase~1} \quad Only univariate log-returns are used, as defined in Eq.~\ref{eq:log_returns}, and no preprocessing is applied to them since no mention of it is made by Kim et al. \cite{kim2025diffusion}, and the average log-return \(\mu=0.0005\) is not conflictual with prior sampling assumptions.
...
the VP formulation is parameterised by $\beta_{\max} = 8$ and $\beta_{\min} = 0.01$. The choice was reached after testing the forward process and reaching prior matching while retaining as much as possible signal, with focus on the first.
...
$P_{\text{std}} \in \{1.0,\,1.2,\,1.4,\,1.6,\,1.8,\,2.0\}$ based on validation loss at first and generative samples later. This was influenced by Phase~1, where small \(t-\)regimes were identified as the hardest to predict. Lower \(P_{\text{mean}}\) concentrate more training samples on low-sigmas and higher \(P_{\text{std}}\) helps cover the tails. Above \(P_{\text{std}}=1.8\) no significant generation gains were observed.

\textbf{Training parameters.} The batch size is 64 across both phases. Phase~1 trains for \(1000\) epochs, as Kim et al.~\cite{kim2025diffusion}, while Phase~2 uses half that budget, which reflects the hardware constraints and the larger number of available training windows, which increases the number of gradient steps per epoch.
