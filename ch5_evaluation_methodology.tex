...
\textbf{Climatology baseline and CRPS skill score} \quad A raw CRPS value is difficult to interpret in isolation, so we construct a \emph{climatology} baseline whose ensemble is formed by drawing, with replacement, from the full pool of observed Close log-returns, ignoring the OHL conditioning signal entirely. The same energy-score estimator is applied to both ensembles for a like-for-like comparison. Relative skill is summarised by the CRPS skill score $\text{SS}_{\text{CRPS}} = 1 - \overline{\text{CRPS}}_{\text{model}} / \overline{\text{CRPS}}_{\text{clim}}$, where $1$ is perfect, $0$ means no improvement over the uninformative baseline, and negative values indicate the model is worse than climatology.
