...
\multirow{3}{2.0cm}{Noise schedule}
  ...
  & \textemdash
  & $P_{\text{mean}}{=}{-1.4},\;P_{\text{std}}{=}1.8$ \\
\midrule
%--- Training ------------------------------------------------------------
...
\multirow{3}{2.0cm}{Sampling}
  ...
  & $\sigma_{\max}=8.0$, $\sigma_{\min}=0.002$\\
\bottomrule
\end{tabular}
\caption{Training setup for Phase~1 (replication of Kim et al.\ \cite{kim2025diffusion})
and Phase~2 (EDM-CSDI). Parameters shared across both phases appear in merged cells. \emph{emb.} is short for embeddings and \emph{wind.} for windows.}
