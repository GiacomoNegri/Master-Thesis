# csdi_train_x0.py — x0-MSE variant of csdi_train_modified.py
# The network predicts clean data x0 directly instead of noise ε.
# Loss: ||x0 - x0_hat||²  (plain MSE in data space, no σ weighting).
# All other structure, arguments, and diagnostics are identical to csdi_train_modified.py.
import os
import math
import json
import random
...
def train(
    ...
    for epoch in range(start_epoch, num_epochs):
        ...
        for batch_idx, batch in pbar:
            ...
            # g(t) for diagnostic plots 09/10 only — not used in x0-MSE loss
            if collect_diag:
                with torch.no_grad():
            ...
            with torch.amp.autocast("cuda", enabled=use_amp):
                # x0-MSE: network output is x0_hat (predicted clean signal)
                x0_hat = model(
                    x_t=x_t,
                    t=t_cont,
                ...
                # Loss in data space: ||x0 - x0_hat||². No sigma weighting — avoids
                # the 1/σ² gradient amplification present in the ε-MSE formulation.
                loss = masked_mse(x0_hat, observed_data, target_mask, None, debug=log_this_batch)

                if log_this_batch:
                    print(f"  x0_hat | norm={x0_hat.norm().item():.4f}  mean={x0_hat.mean().item():.4f}  std={x0_hat.std().item():.4f}  min={x0_hat.min().item():.4f}  max={x0_hat.max().item():.4f}  median={x0_hat.median().item():.4f}")
                    print(f"  x0     | norm={observed_data.norm().item():.4f}  mean={observed_data.mean().item():.4f}  std={observed_data.std().item():.4f}  min={observed_data.min().item():.4f}  max={observed_data.max().item():.4f}  median={observed_data.median().item():.4f}")
                    _a = observed_data.detach().float().view(-1);  _a = _a - _a.mean()
                    _b = x0_hat.detach().float().view(-1);  _b = _b - _b.mean()
                    print(f"  corr(x0, x0_hat) = {float((_a * _b).sum() / (_a.norm() * _b.norm() + 1e-8)):.4f}")
                    # Derived ε̂ for cross-run comparison: ε̂_derived = (x_t - x0_hat) / σ(t)
                    _sigma_b = sigma_t[:, None, None].clamp(min=1e-8)
                    _eps_hat_d = ((x_t - x0_hat) / _sigma_b).detach().float().view(-1)
                    _eps_hat_d = _eps_hat_d - _eps_hat_d.mean()
                    _eps_a = eps.detach().float().view(-1);  _eps_a = _eps_a - _eps_a.mean()
                    print(f"  corr(ε, ε̂_derived) = {float((_eps_a * _eps_hat_d).sum() / (_eps_a.norm() * _eps_hat_d.norm() + 1e-8)):.4f}")

            # Derive ε̂ from x0_hat before backward frees the graph.
            # DiagnosticsCollector expects eps-space inputs; ε̂_derived = (x_t − x0_hat) / σ(t)
            # follows from the VE marginal x_t = x0 + σ·ε  →  ε = (x_t − x0) / σ.
            _eps_hat_derived = (x_t - x0_hat).detach().float() / sigma_t[:, None, None].clamp(min=1e-8)
            _diag_eps_hat = _eps_hat_derived if collect_diag else None

            # Collapse detection — every batch, regardless of debug mode.
            # Fires when std(x0_hat) < 10 % of std(x0): model is collapsing to a constant.
            with torch.no_grad():
                _std_hat = x0_hat.detach().float().std().item()
                _std_x0 = observed_data.float().std().item()
                if _std_hat < 0.1 * (_std_x0 + 1e-8):
                    print(f"  [COLLAPSE step={global_step}]  std(x0_hat)={_std_hat:.4f}  "
                          f"std(x0)={_std_x0:.4f}  ratio={_std_hat / (_std_x0 + 1e-8):.3f}")
        ...
        if val_loader is not None:
            ...
            with torch.no_grad():
                for val_batch_idx, val_batch in enumerate(val_loader):
                    ...
                    # g_t not used in x0-MSE validation loss
                    g_t = None
                    ...
                    with torch.amp.autocast("cuda", enabled=use_amp):
                        ...
                        val_loss = masked_mse(x0_hat, observed_data, target_mask, None, debug=log_this_val_batch)

                        if log_this_val_batch:
                            print(f"  x0_hat | norm={x0_hat.norm().item():.4f}  mean={x0_hat.mean().item():.4f}  std={x0_hat.std().item():.4f}  min={x0_hat.min().item():.4f}  max={x0_hat.max().item():.4f}  median={x0_hat.median().item():.4f}")
                            print(f"  x0     | norm={observed_data.norm().item():.4f}  mean={observed_data.mean().item():.4f}  std={observed_data.std().item():.4f}  min={observed_data.min().item():.4f}  max={observed_data.max().item():.4f}  median={observed_data.median().item():.4f}")
                            _va = observed_data.float().view(-1);  _va = _va - _va.mean()
                            _vb = x0_hat.float().view(-1);         _vb = _vb - _vb.mean()
                            print(f"  corr(x0, x0_hat) = {float((_va * _vb).sum() / (_va.norm() * _vb.norm() + 1e-8)):.4f}")
                            _sigma_bv = sigma_t[:, None, None].clamp(min=1e-8)
                            _eps_hat_dv = ((x_t - x0_hat) / _sigma_bv).float().view(-1)
                            _eps_hat_dv = _eps_hat_dv - _eps_hat_dv.mean()
                            _eps_av = eps.float().view(-1);  _eps_av = _eps_av - _eps_av.mean()
                            print(f"  corr(ε, ε̂_derived) = {float((_eps_av * _eps_hat_dv).sum() / (_eps_av.norm() * _eps_hat_dv.norm() + 1e-8)):.4f}")

                    # Collapse detection — every val batch, regardless of debug mode.
                    _std_hat_v = x0_hat.float().std().item()
                    _std_x0_v = observed_data.float().std().item()
                    if _std_hat_v < 0.1 * (_std_x0_v + 1e-8):
                        print(f"  [COLLAPSE val step={val_batch_idx}]  std(x0_hat)={_std_hat_v:.4f}  "
                              f"std(x0)={_std_x0_v:.4f}  ratio={_std_hat_v / (_std_x0_v + 1e-8):.3f}")
