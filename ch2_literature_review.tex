...
where \(\alpha_t:=1-\beta_t\) and \(\bar{\alpha}_t:=\prod_{s=1}^t\alpha_t\) \cite{ho2020ddpm}. They also shows that by reparametrizing \(x_t(x_0,\epsilon)=\sqrt{\bar{\alpha_t}}x_0+\sqrt{1-\bar{\alpha_t}}\epsilon\), where \(\epsilon \sim \mathcal{N}(0,I)\), the objective function get simplified to a noise prediction loss:
\begin{equation}
...
across the full noise range without requiring separate tuning for each noise level. Its connection with the Wiener Filter are discussed in Sec.\ \ref{app:subsec:wiener}. 
