...
(Eq.~\ref{eq:reverse_sde}). We can also reverse the associated probability flow ODE (Eq.~\ref{eq:prob_flow_ode}), which yields a deterministic process that samples from the same distribution as the SDE. Both the reverse-time SDE and probability flow ODE can be obtained by estimating the score \(\nabla_x\log p_t(x_t)\). Source: Song et al.\ \cite{song2021score}.}
...
\end{equation}
substituting the intractable score with the learned one yields a Neural ODE, where given starting conditions $x_T\sim p_T$ we can deterministically sample, producing a sample from, approximately, \(p_0\). This sampling technique is possibly more efficient.\\
...
The idea of using CSDI for FTS modeling was explored previously by Gihum Kim et al. in \emph{A diffusion-based generative model for financial time series via geometric Brownian motion}. While interesting this work lack the details needed in order to be replicable and provide ambiguous statements about what is actually persue.\\
...
However, an ambiguity in the original paper requires clarification. In Section 3.1, 'Geometric Brownian motion in score-based Models', of \cite{kim2025diffusion} they state:
\begin{quote}
...
is likely a cumulative interpretation: defining \(R_t = X_t - X_0\) as the 
cumulative log-return from the window origin, the VE forward process 
applies to \(R_t\) rather than \(X_t\) directly. Under constant volatility 
\(\nu\), the variance grows linearly as \(\operatorname{Var}(R_t) = \nu^2 
t\), consistent with the VE assumption. This interpretation is not stated 
explicitly in the paper, but is the only reading consistent with both the 
theoretical derivation and the experimental results shown in Figures~4--6 of 
\cite{kim2025diffusion}, even if in disagreement with the training parameters shared by the authors: \(\sigma_{\min}=0.01\) and \(\sigma_{\max}=1.0\) for GBM-style diffusion. The replication in this work proceeds under this assumption.\\
...
Another inconsistency arises between and the described use of time, positional, and diffusion-step embeddings. The text states that
\begin{quote}
...
A more radical alternative is to replace Brownian motion itself with a heavy--tailed jump process. Yoon et al.\ \cite{yoon2023score} propose the L\'evy-It\^o Model, which uses isotropic \(\alpha\)-stable L\'evy processes instead of Brownian motion. They derive the corresponding reverse-time SDE and train the model through fractional denoising score matching. Such an approach is conceptually appealing for financial data, where jumps and rare events are structurally important. However, it requires a different score object, a different reverse process, and a substantially different mathematical framework from the Gaussian EDM--CSDI model used in this work.\\
