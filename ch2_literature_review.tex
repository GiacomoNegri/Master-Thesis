...
\begin{figure}[htbp]
  ...
  \caption{ A sketch of ODE curvature in 1D where \(p_{\text{data}}\) is two Dirac peaks at \(x\pm1\) Horizontal \(t\) axis is chosen to show \(\sigma\in[0,0.25]\) in each plot, with insets showing \ near the data. Example
...
towards data only at small σ. (b) Variance exploding variant has extreme curvature near data and
...
σ → 0, the trajectories become linear and point towards the data manifold. Source: Song et al.\ \cite{song2021score}.}
  \label{fig:sde_diffusion_process}
\end{figure}
