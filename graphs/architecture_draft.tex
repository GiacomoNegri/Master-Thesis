...
\node[transblock, below=of diffadd] (temporal) {
    ...
    \scriptsize $(B,\mathrm{ch},K,L) \to (B\!\cdot\!K,\mathrm{ch},L)$\\[-1pt]
    \scriptsize batch dim $B\!\cdot\!K$, $n_{\mathrm{heads}}=4$ heads
};

\node[transblock, below=of temporal] (feature) {
    \textbf{Feature Transformer}\\
    TransformerEncoder over feature axis $K$\\[-1pt]
    \scriptsize $(B,\mathrm{ch},K,L) \to (B\!\cdot\!L,\mathrm{ch},K)$\\[-1pt]
    \scriptsize batch dim $B\!\cdot\!L$, $n_{\mathrm{heads}}=4$ heads
};

% ADD — Sinusoidal PE nodes: x aligned with sideinfo column, y aligned with diffadd / feature rows
...
% diffusion_emb is centered above diffusion_projection
% (relabelled from c_noise: inside the block the incoming tensor is the
%  DiffusionEmbedding output, not the scalar/c_noise input one level up)
\node[inputblock, above=0.35cm of diffproj] (diffemb) {
    \textbf{diffusion\_emb}\\
    $(B,\mathrm{demb\_dim})$
};
...
% ADD — sinpe (temporal PE): x from sideinfo column, y from diffadd row
\node[inputblock] at ($(sideinfo |- diffproj)+(1.0cm,0)$) (sinpe) {
    \textbf{Sinusoidal PE}\\
    $(B,\mathrm{ch},L)$
};

% ADD — sinpe2 (feature PE): x from sideinfo column, y from feature row
\node[inputblock] at ($(sideinfo |- feature)+(1.0cm,0)$) (sinpe2) {
    \textbf{Sinusoidal PE}\\
    $(1,\mathrm{ch},K)$
};
...
\node[sideout, right=4.2cm of resskipsplit] (skipout) {
    \textbf{skip output}\\
    $(B,\mathrm{ch},K,L)$
};
...
\draw[sidearrow] (diffemb) -- (diffproj);
\draw[sidearrow] (diffproj.east) -- node[above, shapetext, yshift=2pt]{(broad. over $K{\cdot}L$)\\ [-2pt]$(B,\mathrm{ch},1)$} (diffadd.west);

\draw[sidearrow] (sideinfo) -- (condproj);
\draw[sidearrow] (condproj.west) -- node[above, shapetext] {$(B,2\mathrm{ch},K{\times}L)$} (condadd.east);

\draw[skiparrow] (resskipsplit.east) -- node[above, shapetext] {skip\\ $(B,\mathrm{ch},K{\times}L) \to (B,\mathrm{ch},K,L)$} (skipout.west);
...
% ADD — arrows from sinpe (temporal PE) to Temporal Transformer,
%        and from sinpe2 (feature PE) to Feature Transformer
\draw[sidearrow] (sinpe.south) |- (temporal.east);
\draw[sidearrow] (sinpe2.west) -- (feature.east);
...
\node[container,
    fit=(flatten)(diffadd)(temporal)(feature)(midproj)(condadd)(gate)(filter)(multiply)(outproj)(resskipsplit)(reshaperes)(resadd)(diffemb)(diffproj)(sideinfo)(condproj)(skipout)(sinpe)(sinpe2),
