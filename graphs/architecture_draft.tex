...
\node[inputblock] at (sideinfo |- feature) (sinpe2) {
    \textbf{Sinusoidal PE}\\
    $(\cdot,\mathrm{ch},K)$
};
...
\draw[sidearrow] (diffproj.east) -- node[above, shapetext] {$(B,\mathrm{ch},1)$\\[-2pt]\scriptsize(broadcast over $K{\cdot}L$)} (diffadd.west);
