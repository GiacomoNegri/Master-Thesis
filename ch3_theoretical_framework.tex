...
Data are formally represented as \(X \in \mathbb{R}^{K \times L}\), where \(K\) denotes the number of channels and \(L\) the number of time steps per window. In Phase~1, \(K = 1\) and the input reduces to a univariate sequence of Close log-returns.

\(\text{Phase 2}\quad\) OHLC data are formally represented as \(X \in \mathbb{R}^{K \times L}\),
...
The close log-return follows an AR(1) mean with log-GARCH variance: \(r_t = \mu + \phi\, r_{t-1} + \varepsilon_t\) with \(\varepsilon_t = \sigma_t z_t\) and \(z_t \sim t_\nu\), therefore we are modeling:
\[
...
\(\text{Phase 1}\quad\) Phase~1 is identical in its implementation to Song et al. in \cite{song2021score}, but in a simplified version [CITE REPOSITORY].

\(\text{Phase 2}\quad\) For Phase~2, we can look at the forward VE formulation, Eq.\ref{eq:reverse_sde}, the corresponding reverse SDE for VE case is \[dx=-\sigma\nabla_x \log p_\sigma(x)\] when defined in \(\sigma\)-space (with \(\sigma\) decreasing) instead of \(t\)-space, due to EDM conditioning modeling choices. With the deterministic counterparts, the probability flow ODE is:
\[
...
\section{EDM}
The components described in this section apply exclusively to Phase~2. Phase~1 follows the standard Song et al.\ \cite{song2021score} VE--SDE formulation without EDM preconditioning or loss weighting.
...
The CSDI-based backbone described in this section is shared by both phases. The sole phase-specific difference is in the diffusion-step embedding, noted in the Embeddings paragraph below.
...
\(\text{Phase 1}\quad\) The \texttt{unconditional} setting is used for Phase~1, where \(K=1\) and the model is trained on
...
\(\text{Phase 2}\quad\) When \(K>1\) we use \texttt{predict\_close} strategy which is a fixed deterministic mask:
\[
...
\(\text{Phase 1}\quad\) As discussed in Sec.~\ref{sec:scorediffmodels}, the Euler--Maruyama
...
\(\text{Phase 2}\quad\) In the EDM phase, we replace this with the Heun second-order
