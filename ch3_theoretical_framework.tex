...
the EDM framework. These structures are relevant for the Phase 1, which aims to replicate the results of Kim et al.\cite{kim2025diffusion}. Meanwhile, for Phase 2 we will move towards the EDM framework. 

Phase 1 is identical in its implementation to Song et al. in \cite{song2021score}, but in a simplified version [CITE REPOSITORY]. For the second pahse, we can look at the forward VE formulation, Eq.\ref{eq:reverse_sde}, the corresponding reverse SDE for VE case is \[dx=-\sigma\nabla_x \log p_\sigma(x)\] when defined in \(\sigma\)-space (with \(\sigma\) decresing) instead of \(t\)-space, due to EDM conditioning modeling choices. With the deterministic counterparts, the probability flow ODE is:
\[
...
This work employs two masking strategies: \texttt{unconditional} and \texttt{predict\_close}. The \texttt{unconditional} setting is used for Phase 1, where \(K=1\) and the model is trained on 
