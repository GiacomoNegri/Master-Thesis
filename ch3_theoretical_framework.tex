...
The close log-return follows an AR(1) mean with log-GARCH variance:
\begin{align}
    r_t &= \mu + \phi\, r_{t-1} + \varepsilon_t, \qquad \varepsilon_t = \sigma_t z_t, \quad z_t \sim t_\nu, \label{eq:garchx_mean}\\
    \log \sigma_t^2 &= \omega + \alpha \log(\varepsilon_{t-1}^2 + c)
        + \beta \log \sigma_{t-1}^2 + \boldsymbol{\delta}^\top \mathbf{q}_t, \label{eq:garchx_var}
\end{align}
with $|\phi|<0.5$, $\alpha+\beta<0.98$, and $\nu>2$. The log-GARCH
formulation keeps $\sigma_t^2$ positive by construction without sign
constraints on $(\omega,\alpha,\beta)$. The Student-$t$ innovations capture
fat tails parametrically through $\nu$. The OHLC conditioning vector is:
\[
...
(see App.~\ref{app:baselines} for the likelihood and estimation details).

\subsection{Merton-X: Jump-Diffusion with OHLC-Driven Volatility}
\label{sec:mertonx_theoretical}

The close log-return is decomposed into a diffusive and a compound Poisson
component:
\begin{equation}
...
Marginalising over Poisson counts yields a Gaussian mixture likelihood
(App.~\ref{app:baselines}).\\

Both baselines are parametric and trained globally over each ticker's full
history. They capture complementary stylized facts — volatility clustering and
