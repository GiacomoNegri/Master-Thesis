...
The data are therefore partitioned into observed conditioning variables
$X^{\text{co}} = \{O, H, L\}$ and a generation target $X^{\text{ta}} = \{C\}$,
and the model is trained to learn the conditional distribution:
\[
    p\!\left(X^{\text{ta}} \mid X^{\text{co}} = x^{\text{co}}, \tau\right)
\]
where \(\tau\) denotes the temporal conditioning provided by \texttt{observed\_tp}. Unlike \(X^{\text{co}}\), which is masking-strategy dependent, \(\tau\) is always present (Sec.\ \ref{sec:masking_strategies_ch3}).
...
Nevertheless, it remains a fundamental structural property of the data-generating 
process. On the other hand, because the Gaussian diffusion kernel has unbounded 
support, there is no guarantee that generated Close values will respect the implied 
bounds $[L_t, H_t]$. Moreover, enforcing such constraints is not always desirable: 
confining generated price paths strictly within the High and Low bounds would 
severely restrict the diversity of the generated samples. No constraint-preserving 
mechanism is applied in this work; the ordering relationship is further disrupted by 
feature-wise global normalisation, which rescales each channel independently and 
therefore does not preserve the relative ordering between channels.
...
per-day information as the diffusion model's conditioning set. Each model is fitted per ticker by maximum likelihood
via L-BFGS-B\ \cite{byrd1995limited}.
...
(Sec.\ \ref{app:baselines} for more details).
...
It is well established that marginalising over Poisson counts results in a Gaussian mixture likelihood (Sec.\ \ref{app:baselines}). 

Both baselines are parametric and fitted separately on each ticker's full history, and are therefore ticker-specific. Moreover Merton-X captures a complementary stylized fact to GARCH-X: discontinuities. Nevertheless, none of the two learns the conditional distribution $p(r_t^C \mid r_t^O, r_t^H, r_t^L)$ non-parametrically. The diffusion model is the non-parametric alternative,
at the cost of higher complexity.

\section{VE Instantiation for OHLC}
...
the EDM framework. These formulation are relevant for Phase~1, which aims to replicate the results of Kim et al.\cite{kim2025diffusion}. Meanwhile, for Phase~2 we will move towards the EDM framework.

\textbf{Phase 1} \quad Phase~1 is identical in its implementation to Song et al. in \cite{song2021score}, but in a simplified version.

\textbf{Phase 2} \quad For Phase~2, by looking at the forward VE formulation (Eq.\ \ref{eq:forward_ve}), we can derive the reverse SDE\[dx=-\sigma\nabla_x \log p_\sigma(x)\] when defined in \(\sigma\)-space instead of \(t\)-space and with \(\sigma\) decreasing, as by EDM framework. The deterministic counterparts, the probability flow ODE, is:
\[
...
$\mathcal{N}(0, \sigma_{\max}^2 I)$; for this prior to be well-matched 
to the training distribution, the input data must be approximately 
centred at zero with a scale commensurate with $\sigma_{\max}$.
...
is motivated by the GBM theoretical motivations of Sec.\ \ref{sec:gbm_sde_ch2} and by the experimental results shown in Figures~4--6 of Kim et al.\ \cite{kim2025diffusion}, that otherwise wouldn't have any reason to differentiate VE from GBM. How non--stationarity of cumulative log-returns is handled is addressed in Sec.\ \ref{sec:sliding_window_ch4}.

\begin{figure}[htbp]          % placement: here, top, bottom, page
  \centering
  \includegraphics[width=0.9\textwidth]{images/Kim_et_al_architecture.jpg}
  \caption{CSDI-based diffusion architecture. Source: Kim et al.\ \cite{kim2025diffusion}.}
  \label{fig:kim_architecture}
\end{figure}

Another inconsistency in Kim et al.\ \cite{kim2025diffusion} concerns the use of time, positional, and diffusion-step embeddings. The text states that
\begin{quote}
...
\end{quote}
However, Fig.\ \ref{fig:kim_architecture} clearly shows that the time embeddings is provided only after the Transformer layer. This last interpretation was used. Moreover, based on the model architecture presented in Fig.\ \ref{fig:kim_architecture}, the diffusion-step, temporal, and positional embeddings were interpreted as being injected independently at every residual block, consistently with the approach adopted in CSDI\ \cite{tashiro2021csdi}. Lastly, Kim et al.\ \cite{kim2025diffusion} describe the time embedding as combining sinusoidal functions with learnable vectors, while the positional embedding relies solely on fixed sinusoidal functions with no learnable parameters; this distinction suggests that the positional embedding is the only one among the three that contains no trainable weights. 

\section{EDM}
The components described in this section apply exclusively to Phase~2.

\textbf{Preconditioning} \quad Training a raw network \(F_\theta (x_t;\sigma)\) directly as a denoiser is impractical because according to the VE forward process (Eq.\ \ref{eq:forward_ve}) the inputs have variance \(\sigma_{\text{data}}^2+\sigma^2\), which means that the network sees input that varies by orders of magnitude across the distribution of \(\sigma\) (different noise levels), as discussed in Sec.\ \ref{sec:edm_literature}. EDM addresses both problems introducing preconditioned denoiser \(D_\theta\) (Eq.\ \ref{eq:edm_denoiser}). 
...
\begin{itemize}
    ...
    \item \(c_{\text{skip}}(\sigma)\), skip connection: is the Wiener filter coefficient (Sec.\ \ref{app:subsec:wiener}), or the optimal linear estimator of \(x_0\) given \(x_t\). This is dervied from the MMSE argument, and it controls how much of the noisy input is passed directly to the output.
...
\begin{figure}[htbp]          % placement: here, top, bottom, page
  \centering
  \includegraphics[width=0.6\textwidth]{images/preconditioning.jpg}
  \caption{Preconditioning coefficients for \(\sigma_{\text{data}}=1\), with \(\sigma_{\min}=0.002\) and \(\sigma_{\max}=8.0\).}
...
\textbf{Sigma distribution} \quad The EDM framework was originally calibrated on ImageNet \cite{deng2009imagenet} 
...
\textbf{Loss function} \quad EDM varies also the loss function characterization by moving away from noise prediction as in Phase~1 (Eq.\ \ref{eq:csdi_objective}), to a "clean" data prediction as in Eq.\ \ref{eq:edm_loss} and with different weighting, as discussed in Sec.\ \ref{sec:edm_literature}.
...
the locality assumptions of convolutions is a poor fit. Transformers, by 
...
Fig.\ \ref{fig:csdi_overview} illustrates the overall flow of the denoiser. The noise level \(\sigma\), the noised data \(x_t\), the observed (conditioning) data, the \texttt{cond\_mask} indicating which entries serve as conditioning inputs, and the \texttt{observed\_tp} time indices are all injected into the network at the stages depicted in Fig.\ \ref{fig:model_input}. These inputs are processed through a stack of \(n\) \texttt{ResidualBlock} layers, producing the preconditioned output \(F_\theta\bigl(c_{\text{in}}(\sigma)x;\, c_{\text{noise}}(\sigma)\bigr)\). The internal structure of each \texttt{ResidualBlock}, shown in Fig.\ \ref{fig:csdi_residual_block}, is described in detail below.

% % THE GRAPHS ARCHITECTURES ARE HERE

%DO NOT PRINT \input{graphs/overview_flow}

\input{graphs/overview_flow_draft}

% DO NOT PRINT \input{graphs/overview}

\input{graphs/architecture_draft}

% DO NOT PRINT\input{graphs/architecture}
...
\begin{figure}[htbp]
  ...
  \begin{minipage}[t]{0.68\textwidth}
    ...
    \caption{Representation of model inputs and how they are feeded into the Denoiser. \texttt{input projection} and \texttt{ResidualBlock} are those represented in Fig.\ \ref{fig:csdi_overview}.}
...
residual layer rather than shared across them, Fig.\ \ref{fig:csdi_residual_block}. Weight sharing would 
constrain all layers to operate at the same level of abstraction, 
limiting the model's ability to build hierarchical representations, while independent weights allow each layer to specialise; consistently with CSDI implementation\ \cite{tashiro2021csdi}. Multi-head attention\ \cite{vaswani2017attention} allows the model to attend to multiple 
dependency patterns simultaneously within each axis, for instance it could modeled 
short-range trend and longer-range volatility clustering along the temporal axis, or spread relationships along the feature axis. The structure is represented in Fig.\ \ref{fig:transformer_architecture}. 

\textbf{Gated residual blocks} \quad Following the WaveNet structure 
\cite{oord2016wavenet}, each block applies the element wise multiplication of \(\sigma(\cdot) \odot 
\tanh(\cdot)\). The 
sigmoid \(\sigma(\cdot)\) acts as the selection gate, producing a value 
in \([0,1]\) that controls \emph{how much} of the candidate update is passed 
...
while preserving significant features. This could be useful for FTS, since Signal-to-Noise Ratio (SNR) is often low and depends on the regime. 
...
\emph{diffusion-step embedding} (\texttt{diffusion\_emb}: sinusoidal basis projected through two 
...
phase, telling the network the current noise level. The \emph{side information embeddings} (\texttt{side\_info})
...
\emph{sinusoidal positional encoding} (\texttt{Sinusoidal PE}), following Kim et al.\ \cite{kim2025diffusion}, 
...
\textbf{Residuals and skip connections} \quad Each \texttt{ResidualBlock} produces two outputs: a residual branch, added back to the block's input, and a skip branch, collected and pooled across all \(n\) blocks before the final output projection. Following WaveNet\ \cite{oord2016wavenet} and CSDI\ \cite{tashiro2021csdi}, each \texttt{ResidualBlock} produces two outputs: a residual branch, added back to the block's own input, and a skip branch, aggregated across all \(n\) blocks before the final output projection. This design allows every block to contribute directly to the prediction, irrespective of its depth in the stack. The residual addition and the skip aggregation are rescaled by \(1/\sqrt{2}\) and \(1/\sqrt{n}\), respectively, to maintain stable signal variance as representations accumulate through successive blocks. The \texttt{mid\_projection} is a pointwise convolution that doubles the channel dimension immediately before the side-information conditioning is added; this provides the extra capacity needed to later split the combined representation into the gate and filter branches of the gated activation unit.

\section{Masking Strategies}
\label{sec:masking_strategies_ch3}
...
\textbf{Remark on the \texttt{unconditional} label} \quad It is worth stressing that the \texttt{unconditional} mask eliminates value-level conditioning, since the OHL channels are zeroed in the model input, but it does not eliminate temporal conditioning. The time embedding (\texttt{observed\_tp}), encoding the absolute calendar position of each time step as described in Sec.\ \ref{sec:model_architecture}, is passed to the model regardless of mask mode and contributes a form of conditioning on calendar time. In this sense, both training phases mdoel a temporally conditioned distribution \(p_\theta(x \mid \tau)\), where \(\tau\) denotes \texttt{observed\_tp}; the \texttt{unconditional} mask differs from \texttt{predict\_close} only in not providing observed OHLC values.

\textbf{Phase 2} \quad When \(K>1\) we use the \texttt{predict\_close} strategy that defines a deterministic mask:
\[
...
stochastically at each training step. The \texttt{predict\_close} mask is consistent with 
...
the conditioning signal. The conditional EDM loss therefore (Eq.\ \ref{eq:edm_loss}) becomes:
\[
...
hundreds to thousands of steps, implying a high number of NFEs. Consistently with Song et al.\ \cite{song2021score} the probability flow ODE (Eq.\ \ref{eq:probability_flow}) is used in Phase~1, where the learned score \(s_\theta(x_t,t)\) is substituted for the 
intractable \(\nabla_x \log p_t(x_t)\).

\textbf{Phase 2} \quad In the EDM phase, the Heun second-order
predictor--corrector sampler (Eq.\ \ref{eq:edm_sampler}) is used, which
integrates the probability flow ODE rather than the reverse SDE. The 
corrector step requires double the number of NFEs per 
