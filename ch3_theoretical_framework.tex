...
\textbf{Preconditioning} \quad Training a raw network \(F_\theta (x_t;\sigma)\) directly as a denoiser is impractical because according to 
...
\textbf{Loss function} \quad EDM varies also the loss function characterization by moving away from noise prediction as in Eq.\ref{eq:csdi_objective}, to a "clean" data prediction, Eq.\ref{eq:edm_loss}, with different weighting, as discussed in Sec.\ref{sec:edm_literature}.
...
Fig.~\ref{fig:csdi_overview} illustrates the overall flow of the denoiser. The noise level \(\sigma\), the noised data \(x_t\), the observed (conditioning) data, the \texttt{cond\_mask} marking which entries are conditional, and the \texttt{observed\_tp} time indices are fed into the network at the stages shown in Fig.~\ref{fig:model_input}. These inputs are processed by a stack of \(n\) \texttt{ResidualBlock} layers, yielding the preconditioned output \(F_\theta\bigl(c_{\text{in}}(\sigma)x;\, c_{\text{noise}}(\sigma)\bigr)\). The internal structure of each \texttt{ResidualBlock}, shown in Fig.~\ref{fig:csdi_residual_block}, is detailed below.
...
short-range trend and longer-range volatility clustering along the temporal axis, or spread relationships along the feature axis. The detailed structure is represented in Fig.\ \ref{fig:transformer_architecture_encoder}. 
...
while preserving significant features. This could be useful for FTS, since Signal-to-Noise Ratio (SNR) is often low and depends on the regime, training window. 
...
\emph{sinusoidal positional encoding} (\texttt{Sinusoidal PE}), following \cite{kim2025diffusion}, 
...
\textbf{Residuals and skip connections} \quad Each \texttt{ResidualBlock} produces two outputs: a residual branch, added back to the block's input, and a skip branch, collected and pooled across all \(n\) blocks before the final output projection. Following WaveNet \cite{oord2016wavenet} and CSDI \cite{tashiro2021csdi}, this lets every block contribute directly to the prediction regardless of its depth in the stack, while the residual sum and the skip pooling are rescaled by \(1/\sqrt{2}\) and \(1/\sqrt{n}\) respectively to keep the signal variance stable as blocks accumulate. The \texttt{mid\_projection} is a pointwise convolution that doubles the channel dimension immediately before the side-information conditioning is added; this provides the extra capacity needed to later split the combined representation into the gate and filter branches of the gated activation unit.
