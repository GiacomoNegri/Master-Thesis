...
def main():
    ...
    p.add_argument("--start-date", type=str, default="2020-01-02")
    ...
    # ---- Load data ----
    # Example: enforce your exact feature order (must match how you trained on single ticker)
    COLS = ["Open", "High", "Low", "Close", "Volume"]  # adjust to your actual processed columns
    dfs = load_all_tickers("./data/sp500_individual", cols=COLS, start_date=start_date, end_date=end_date)
    ...
    def create_dataloaders_multi(dfs, seq_len, val_rat, batch_size, close_idx=3, forecast_horizon=None):
        ...
        for df in dfs_list:
            ...
            # keep only if each side can form at least one window
    ...
    if execute_train:
        ...
        for epoch in range(epochs):
            ...
            progress = tqdm(train_loader, desc=f"Epoch {epoch}", total=len(train_loader))
        ...
        final_ckpt = checkpoints_dir / f"FINAL_scratch_ckpt_epoch_{epochs-1}_{today_str}.pt"
        torch.save(
