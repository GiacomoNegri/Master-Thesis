...
# --- DDP: train() accepts local_rank, is_main, and train_sampler ---
def train(
    config: Dict[str, Any],
    train_loader,
    val_loader=None,
    resume_checkpoint: Optional[str] = None,
    train_sampler=None,   # DistributedSampler — needed to call set_epoch each epoch
    local_rank: int = 0,
    is_main: bool = True,
) -> None:
    ...
    # Same seed for model weight initialisation across all ranks (DDP will sync anyway)
    set_seed(int(config["train"]["seed"]))

    # 1) Instantiate diffusion processes
    processes = Diffusion_Processes(config["process"])

    # 2) Instantiate model and move to this rank's GPU
    ...
    # 5) Output dir — only rank 0 creates it
    ...
    use_wandb = bool(wandb_cfg.get("project")) and is_main
    if use_wandb:
    ...
    # Early stopping state — only rank 0 makes the decision
    ...
    # Signal used by all ranks to break the epoch loop on early stop
    stop_signal = torch.zeros(1, device=device)

    for epoch in range(start_epoch, num_epochs):
        model.train()

        # Tell the sampler which epoch we're in so shuffling differs per epoch
        ...
        epoch_loss_count = 0
        ema_loss = None
        _grad_buf = []   # grad norm per batch — epoch-end stats
        _loss_buf = []   # step loss per batch — for Pearson r with grad norm
        ...
        if val_loader is not None:
            ...
            # Average val loss across all ranks via all-reduce
        ...
        # Checkpoint by epoch cadence — rank 0 only
        if is_main:
...
if __name__ == "__main__":
    ...
    # Build the base dataset once (shared across all ranks)
    ...
    if val_split_ratio is not None:
        ...
        # Each rank evaluates a disjoint shard of the val set
    ...
    # Carve out training subset from remaining indices
    if subset_size is not None:
    ...
    # DistributedSampler splits the training data across ranks
