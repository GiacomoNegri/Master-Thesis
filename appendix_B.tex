...
Ch.\ \ref{ch:literature_review}  introduced the Fisher divergence as the natural objective for
...
\textbf{Intuition} \quad Treat $p_t(x)$ as a cloud of probability mass. Two forces
...
\textbf{The FP equation} \quad For the forward SDE (Eq.\ \ref{eq:generalsde}) with
...
\textbf{Deriving the probability flow ODE} \quad We seek a deterministic ODE
...
\end{equation}
which is the probability flow ODE stated in Sec.\ \ref{sec:scorediffmodels}.
...
% ODE (Ch.~3). A single step from $\sigma_i$ to $\sigma_{i+1}$ (step size
...
% The VP SDE from Sec.\ \ref{sec:scorediffmodels}
...
Sec.\ \ref{sec:fts_literature} introduced the CRPS as a calibration-adjusted
